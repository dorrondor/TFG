...
static struct nftnl_rule *setup_rule(uint8_t family, const char *table,
                   const char *chain, const char *handle, int iof, uint8_t proto, uint16_t port, uint8_t addr)
{
...
int addRul(int argc, char *argv[])
{
	...
	if(argc==8){
        ...
        port = htons(tport);
        //printf(agrv[8]);
        inet_pton(AF_INET, argv[8], addr);
    }
    
    // Now creating rule
    if (argc != 9)
        r = setup_rule(family, argv[2], argv[3], NULL, iof, proto, port, addr);
    else
        r = setup_rule(family, argv[2], argv[3], argv[4], iof, proto, port, addr);
